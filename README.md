...
| 4/5 套件 | 這台電腦的 NuGet 快取已經有就直接用；沒有才**問你要不要下載**（約 17 MB） | 略過 |
...
- 還原 M 之後，Desktop 會出現「暫止的變更」提示，按套用才會生效；只想動量值就請 AI「只還原量值」
...
| Power BI 顯示「查詢中有暫止的變更尚未套用」 | M 被 API 改過。按「套用」即可更新；套用後確認 M 是你要的版本，不是的話到進階編輯器貼上正確的 M 再「關閉並套用」 |
